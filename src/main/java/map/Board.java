...
public class Board {
    ...
    public List<Move> getLegalMoves(Player player, boolean overrideMoves) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                TODO: if (reachableField[y][x] == MapAnalyzer.REACHABLE) {
    ...
    public Move getLegalMove(int x, int y, Player player) {
        ...
        } else if (player.hasOverrideStone() && "x12345678".indexOf(field[y][x]) != -1) {
            legalMove = checkMove(x, y, player.getCharNumber(), true);
        }
    ...
    public void colorizeMove(Move legalMove, Player player, int additionalOperation) {
        ...
        for (int[] position : legalMove.getList()) {
            ...
            field[colorizeY][colorizeX] = player.getCharNumber();
        ...
        if (inversion) {
            ...
            choice(player.getCharNumber(), (char) (additionalOperation + '0'));
        } else if (bonus) {
