...
public class BRSPlus {
    ...
    private int coinParityHazard = 5000;
    ...
    public BRSPlus(Board board, Player[] players, MapAnalyzer mapAnalyzer, AnalyzeParser analyzeParser) {
        this.board = board;
        ...
        timeToken = new Token();
        BRSPlus.analyzeParser = analyzeParser;
    ...
    private Move startSearching(List<BoardMove> executedStartMoves, int searchDepth, int nextPlayerNum,
        ...
        for (BoardMove boardMove : executedStartMoves) {
            mapsAnalyzed++;
            int tmpValue = searchMove(nextPlayerNum, ourPlayerNum, boardMove.getBoard(),
                    boardMove.getMove(), count, depth, searchDepth, maxLoop, alpha, beta);
            if (tmpValue > value) {
                value = tmpValue;
    ...
    private int searchMove(int currPlayer, int ourPlayerNum, Board board, Move move, int count, int depth,
                           int maxDepth, int maxLoop, int alpha , int beta) throws TimeExceededException {
        ...
        if (playerMoves.isEmpty() || player.isDisqualified()) {
            ...
            if (noMoves) {
                ...
                if(maxLoop < numPlayers) {
                    maxLoop++;
                    return searchMove(nextPlayer, ourPlayerNum, board, move, count,depth - 1, maxDepth, maxLoop,
                            alpha, beta);
        ...
        if (orderMoves && depth < (maxDepth - 1) && player == ourPlayer) sortExecutedMoves(executedMoves, ourPlayer, player);
        if (player != ourPlayer) {
            sortExecutedMoves(executedMoves, ourPlayer, player);
            specialMove = executedMoves.get(0).getMove();
            specialBoard = executedMoves.get(0).getBoard();
        }
        ...
        if (count >= 2 && player != ourPlayer) {
           value = searchMove(nextPlayer, ourPlayerNum, specialBoard, specialMove, count,
                    depth, maxDepth, maxLoop,  alpha, beta);

        } else {
            for (BoardMove boardMove : executedMoves) {
                mapsAnalyzed++;
                int newCount = count;
                if (player != ourPlayer && boardMove.getMove() != specialMove) newCount++;
                if(player == ourPlayer) newCount++;
                int tmpValue = searchMove(nextPlayer, ourPlayerNum, boardMove.getBoard(), boardMove.getMove(), newCount,
                        depth, maxDepth, maxLoop,  alpha, beta);
